...
export default async function DashboardPage() {
  ...
  const { data: invoiceRows } = await supabase
    ...
    .limit(3)
  const recentInvoices = (invoiceRows ?? []) as Invoice[]
    ...
    <div className="p-8 space-y-6">
      ...
      >
        ...
        <KpiCard
          label="Open invoices"
          value={pendingCount}
          hint={pendingCount === 0 ? 'You’re all paid up' : 'Awaiting payment'}
        />
      </section>
                ...
                {recentInvoices.map((inv) => (
                  <li key={inv.id} className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-subtle capitalize">{inv.status}</p>
                    </div>
