# Maintenance & Service Tasks — Product & Engineering Spec
...
breakroom microwave dies, a tile cracks in 312. These tasks span hours to
...
**Routes** (`/tasks` group, sidebar nav between IT Hub and Properties):

1. **Board page** (`/tasks`) — Kanban with four columns: **Open**, **In
...
2. **New task page** (`/tasks/new`) — the capture flow:
   ...
   - Submit → create task in `open` status, redirect to detail.
3. **Task detail page** (`/tasks/[id]`) — full media gallery, status
...
5. **Notifications**: when a task is assigned to you, when a high/urgent
   task is created at a property you manage. Email via Resend; in-app
...
- **Reports** — top categories by property, average time-to-close, repeat
  offender rooms (rooms with >N tasks in 90 days — the rooms management
  needs to renovate).
...
`supabase/migrations/<timestamp>_tasks.sql`:
...
create table public.tasks (
  ...
  reference text not null,  -- human-friendly: TSK-0001 per property
  title text not null,
  description text,
...
-- Attachments live in R2 under <property.r2_prefix>/_tasks/<task_id>/<filename>.
...
create table public.task_attachments (
  ...
  caption text,
  -- 'before' (created with the task), 'progress' (added during work),
...
create policy task_attachments_select_org on public.task_attachments for select
  using (org_id = public.current_org_id() or public.is_platform_admin());
create policy task_comments_select_org on public.task_comments for select
  using (org_id = public.current_org_id() or public.is_platform_admin());
create policy task_activity_select_org on public.task_activity for select
  using (org_id = public.current_org_id() or public.is_platform_admin());
create policy task_tags_select_org on public.task_tags for select
  using (org_id = public.current_org_id() or public.is_platform_admin());
```
...
src/app/(app)/tasks/
  ...
  _lib/
    ...
    reference.ts                next-reference helper (TSK-0001 per property)
```
...
The `_tasks/` prefix is hidden from `/media` listing (same exclusion list
as `_meta/`, `_posters/`, `_it-docs/`).

### Capture flow (mobile-first)

1. User taps **New task** → form mounts a `<CaptureUploader>`.
...
5. On submit, the server action receives a list of `{ r2_key, kind,
   poster_key?, content_type, size }` and inserts `task_attachments` rows
   in the same transaction as the `tasks` row.

### Reference numbering

`tasks.reference` is `TSK-NNNN` per property (so each property starts at
`TSK-0001`, mirroring how `events.reference` works). Implementation: a
`generate_task_reference(property_id)` SQL function that selects
`max(substring(reference from 5)::int)` for the property and returns
`TSK-<n+1, zero-padded>`. Server action calls it before insert.
...
- "Mark done" requires at least one attachment with `phase = 'after'`.
  ...
  one-line note (`task_activity.kind = 'force_done'`).
...
| Notification spam                                       | Throttle: at most one email per assignee per task per 5 min; coalesce on the worker side.            |
